...
- **消息分类**：权限 / 选择 / 错误 / 空闲 / 任务完成，各有不同音效
  - 权限确认 → Hero 声 + 对话框，带「允许」和「去终端处理」两个按钮
    - 点「允许」自动切回终端并模拟 `1 + Enter`（默认选第一项，免手动）
    - 点「去终端处理」只切回终端，不代你选
  - 等你选项 → Ping 声 + 对话框
  - 错误/限流 → Basso 声 + banner
  - 空闲等待 → Glass 声 + banner，同会话只响第一次
  - 任务完成 → Funk 声 + banner
- **通知标题带项目名**：「Claude Code · 豆瓣转notion」，多会话一眼分清
- **Claude 图标**：对话框左侧用 Claude 官方 logo
- **不抢焦点**：对话框后台弹，你手上的事不会被打断；只有点按钮才激活终端

> ⚠️ 「允许」按钮首次使用时 macOS 会请求**辅助功能**权限（System Events → 辅助功能），必须授予，否则按键模拟失败。路径：系统设置 → 隐私与安全性 → 辅助功能。对 VS Code 集成终端来说，授权对象是 `Code`（Visual Studio Code）。
